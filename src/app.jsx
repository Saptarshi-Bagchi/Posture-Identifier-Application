...
export default function App() {
  ...
  return <div className={`flex h-screen overflow-hidden pb-20 transition-colors duration-300 ease-in-out lg:pb-0 ${darkMode ? 'theme-dark' : 'theme-light'}`}><Sidebar activeTab={activeTab} onNavigate={setActiveTab} mqttStatus={status} /><main className="h-full min-w-0 flex-1 overflow-hidden px-5 py-6 transition-colors duration-300 ease-in-out sm:px-8 lg:px-10"><div className="mx-auto flex h-full min-h-0 max-w-6xl flex-col"><TopBar title={activeTab} darkMode={darkMode} onThemeChange={setDarkMode} /><div ref={tabContentRef} className={`mt-6 min-h-0 flex-1 ${isScrollable ? 'almanac-scroll overflow-y-auto pr-2' : 'overflow-hidden'}`}><ErrorBoundary key={activeTab}><Page data={ispaData} telemetry={telemetry} history={history} classification={classification} goodPosture={goodPosture} telemetryStatus={status} /></ErrorBoundary></div></div></main></div>
}
