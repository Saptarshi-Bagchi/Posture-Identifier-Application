  ...
  const generatePlan = async (analysis) => {
    ...
    try {
      ...
      if (window.electronAPI?.generatePosturePlan) {
        ...
        const response = await fetch('/api/posture-plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ angle: analysis.angle, category: analysis.category }) })
