  ...
  const generatePlan = async (analysis) => {
    ...
    try {
      ...
      if (window.electronAPI?.generatePosturePlan) {
        plan = await window.electronAPI.generatePosturePlan({ angle: analysis.angle, category: analysis.category })
      } else {
        const response = await fetch('/api/generate-posture-plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ angle: analysis.angle, category: analysis.category }) })
      ...
      if (!Array.isArray(plan) || plan.length !== 7) throw new Error('AI returned an incomplete improvement plan.')
