...
export default function CalibratePage() {
  ...
  const generatePlan = async (analysis) => {
    ...
    setPlanError('')
    try {
      if (aiPlanAvailable === false) throw new Error('Gemini plan generation requires an API key — check your .env file.')
      const plan = await window.electronAPI?.generatePosturePlan?.({ angle: analysis.angle, category: analysis.category })
      if (!Array.isArray(plan) || plan.length !== 7) throw new Error('Gemini plan generation requires an API key — check your .env file.')
