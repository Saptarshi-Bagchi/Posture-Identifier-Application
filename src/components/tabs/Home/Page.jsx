...
function HomeSections() {
  ...
  return <section className="flex h-[calc(100vh-6rem)] min-h-0 shrink-0 flex-col justify-center rounded-3xl border border-slate/20 bg-panel p-4 sm:p-6 lg:p-8"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-mauve">How it works</p><h2 className="mt-3 font-display text-2xl font-bold leading-tight text-navy sm:text-4xl">A calmer way to sit well.</h2><p className="mt-4 max-w-lg text-xs leading-5 text-slate">Three quiet steps turn posture awareness into a habit that fits your workday.</p></div><div className="mt-6 border-t border-slate/20">{steps.map(([number, title, description], index) => <article key={number} className={`relative grid gap-4 border-b border-slate/20 py-8 last:border-b-0 last:pb-0 md:grid-cols-[150px_minmax(0,1fr)] md:gap-8 ${index % 2 === 1 ? 'md:grid-cols-[minmax(0,1fr)_150px] md:text-right' : ''}`}><span aria-hidden="true" className={`font-display text-7xl font-bold leading-none text-mauve/35 ${index % 2 === 1 ? 'md:col-start-2 md:row-start-1' : ''}`}>{number}</span><div className={`${index % 2 === 1 ? 'md:col-start-1 md:row-start-1 md:pr-8' : 'md:col-start-2 md:pl-8'}`}><h3 className="font-display text-2xl font-bold text-navy sm:text-3xl">{title}</h3><p className={`mt-3 max-w-md text-sm leading-6 text-slate md:max-w-lg ${index % 2 === 1 ? 'md:ml-auto' : ''}`}>{description}</p></div></article>)}</div></section>
}

export default function HomePage({ onNavigate }) {
  return (
    <div className="tab-scroll flex min-h-full flex-col gap-4 overflow-y-auto pb-4">
      <section className="relative grid min-h-[calc(100vh-6rem)] flex-1 items-center gap-8 overflow-hidden rounded-[2rem] border border-slate/20 bg-offwhite p-10 sm:p-12 lg:grid-cols-[minmax(0,0.9fr)_minmax(360px,1.1fr)] lg:p-16">
        ...
        <div className="relative z-10 min-w-0">
          ...
          <p className="mt-4 max-w-md text-sm leading-6 text-slate sm:text-base">I-SPA watches your alignment in real time and gives you a gentle nudge before a small slouch becomes a long day of tension.</p>
          <div className="mt-6 flex flex-wrap gap-3"><PillButton icon="wifi" onClick={() => onNavigate('Device Connection')}>Start Tracking</PillButton><PillButton icon="book" variant="secondary" onClick={() => onNavigate('Posture Almanac')}>Learn More</PillButton></div>
          <div className="mt-7 flex flex-wrap gap-7 border-t border-slate/20 px-1 pt-4"><div className="rounded-2xl border border-slate/20 bg-surface/40 px-4 py-2.5"><p className="font-display text-2xl font-bold text-mauve">24/7</p><p className="text-xs font-semibold text-slate">gentle awareness</p></div><div className="rounded-2xl border border-slate/20 bg-surface/40 px-4 py-2.5"><p className="font-display text-2xl font-bold text-mauve">4</p><p className="text-xs font-semibold text-slate">live angle signals</p></div></div>
        </div>
        <HelixVisual />
      </section>
      <HomeSections /><section className="flex h-[calc(100vh-6rem)] min-h-0 shrink-0 flex-col justify-center rounded-3xl border border-slate/20 bg-panel px-4 pt-4 pb-0 sm:px-6 sm:pt-6 sm:pb-0 lg:px-8 lg:pt-8 lg:pb-0"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-mauve">Our team</p><h2 className="mt-3 font-display text-2xl font-bold leading-tight text-navy sm:text-4xl">The people behind I-SPA.</h2><p className="mt-3 max-w-lg text-xs leading-5 text-slate">A small team building tools to help people sit, stand, and move better.</p></div><div className="mx-auto mt-8 w-full max-w-5xl divide-y divide-slate/20 border-t border-slate/20">{[['SB', 'Sainandan Bose', 'Team Leader'], ['MJA', 'Mohammed Junaid Abedin', 'Embedded and IoT engineer'], ['SB', 'Saptarshi Bagchi', 'Software Developer'], ['SU', 'Sobhini Upadhaya', 'Public Relationship Manager'], ['SG', 'Sayan Ghosh', 'UI/UX Designer'], ['TB', 'Tejaswini Baral', 'Lead Researcher']].map(([initials, name, role]) => <article key={name} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-baseline sm:justify-between sm:gap-8"><div className="flex min-w-0 items-center gap-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-mauve/15 font-display text-[11px] font-bold text-mauve">{initials}</div><h3 className="font-display text-base font-bold tracking-tight text-navy sm:text-lg">{name}</h3></div><p className="pl-12 text-xs font-bold uppercase tracking-[0.12em] text-mauve sm:pl-0">{role}</p></article>)}</div></section>
    </div>
  )
}
