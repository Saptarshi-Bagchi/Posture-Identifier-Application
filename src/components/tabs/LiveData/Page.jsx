...
export default function LiveDataPage({ telemetry, history, classification, telemetryStatus, darkMode, onNavigate }) {
  ...
  return <div className="tab-scroll flex h-full min-h-0 flex-col gap-6 overflow-y-auto pr-1"><section className="relative grid h-[calc(100vh-6rem)] min-h-0 min-w-0 flex-none items-center justify-center gap-8 rounded-3xl border border-slate/25 bg-surface p-4 shadow-sm sm:p-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(360px,1.1fr)] lg:p-8"><div className="pointer-events-none absolute -left-10 -top-14 h-32 w-32 rounded-full bg-mauve/10" /><div className="relative min-w-0"><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-mauve/30 bg-mauve/10 px-3 py-1.5 text-xs font-bold text-mauve"><span className="h-2 w-2 rounded-full bg-mauve" /> Live Posture</div><h1 className="max-w-xl font-display text-5xl font-extrabold leading-[0.95] tracking-tight text-navy sm:text-6xl">Sit right.<br /><span className="text-mauve">Feel right.</span></h1><p className="mt-5 max-w-md text-sm leading-6 text-slate sm:text-base">See your alignment in real time, understand the angles behind your posture, and get a gentle nudge when you start to slouch.</p></div><div className="relative min-w-0 rounded-3xl border border-divider/30 bg-surface p-4 sm:p-6"><div className="flex items-center justify-between text-foreground"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-muted">Live posture</p><p className="mt-1 font-display text-2xl font-bold">{classification?.label || 'Good posture'}</p></div><span className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold ${connected ? 'bg-mauve text-offwhite' : 'bg-muted/15 text-muted'}`}><span className="h-2 w-2 rounded-full bg-current" />{connected ? 'Tracking' : 'Waiting'}</span></div><div className="mt-5 grid min-h-[220px] grid-cols-[minmax(0,1fr)_140px] items-center gap-3 rounded-2xl border border-divider/25 bg-panel p-4"><div className="flex h-full items-center justify-center rounded-2xl border border-divider/20 bg-surface"><img src={postureImage} alt={`${classification?.label || 'Good posture'} reference`} className="max-h-44 max-w-full object-contain" /></div><div className="space-y-3"><div className="rounded-2xl bg-navy p-3 text-offwhite"><p className="text-[10px] font-bold uppercase tracking-wider text-offwhite/60">Status</p><p className="mt-1 text-sm font-bold">{connected ? 'On track' : 'Ready'}</p></div><div className="rounded-2xl bg-slate p-3 text-offwhite"><p className="text-[10px] font-bold uppercase tracking-wider text-offwhite/70">Updated</p><p className="mt-1 text-xs font-bold">{updated ? new Date(updated).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : '—'}</p></div></div></div><div className="mt-4 grid grid-cols-2 gap-3">{axes.map(([key, label]) => <div key={key} className="rounded-2xl border border-divider/25 bg-panel px-4 py-4 text-foreground transition-colors duration-300"><p className="text-xs font-bold uppercase tracking-wider text-muted">{label}</p><p className="mt-2 text-lg font-bold transition-all duration-300 ease-out">{Number.isFinite(telemetry?.[key]) ? telemetry[key].toFixed(2) : '—'}</p></div>)}</div></div></section><section className={`${card} flex min-h-[540px] min-w-0 flex-none flex-col overflow-visible p-5 sm:p-7`}><SectionTitle eyebrow="Rolling window" title="Sensor history" /><div className="live-charts-grid mt-4 grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-5 sm:grid-cols-2">{axes.map(([axis, label, color]) => <AxisChart key={axis} axis={axis} label={label} color={color} history={history} />)}</div></section></div>
}
