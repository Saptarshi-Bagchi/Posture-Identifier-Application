...
export default function PostureAlmanacPage({ data }) {
  return <div className="space-y-6"><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{data.postureAlmanac.map((item) => <article key={item.logic} className={`${card} overflow-hidden`}><div className="flex items-center justify-between"><span className={`text-xs font-bold uppercase ${statusColors[item.tone]}`}>● {item.tone}</span></div><div className="mt-3"><PostureImage image={item.image} imageName={item.imageName} name={item.name} /></div><h2 className="mt-4 text-lg font-bold">{item.name}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{item.description}</p><code className="mt-4 inline-flex rounded-lg border border-brand-border bg-brand-navy px-2.5 py-1.5 text-[11px] text-brand-yellow">{item.logic}</code></article>)}</div><section className={card}><SectionTitle eyebrow="Sensor reference" title="Angle definitions & thresholds" action={<Icon name="book" size={22} />} /><p className="text-sm text-slate-400">The ESP32 sends two pitch and two roll angles. ISPA classifies them using the firmware thresholds.</p></section></div>
}
